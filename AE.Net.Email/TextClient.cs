	...
	public abstract class TextClient : IDisposable {
		...
		public virtual void Connect(string hostname, int port, bool ssl, System.Net.Security.RemoteCertificateValidationCallback validateCertificate) {
			try {
				...
				_Stream = _Connection.GetStream(); //IO.Stream from Email Server;
				if (ssl) {
		...
		protected virtual void SendCommand(string command) {
			...
			_Stream.Write(bytes, 0, bytes.Length); //wrtie to email server;
		}
		...
		protected virtual string GetResponse() {
            int max = 0;
            string stream = _Stream.ReadLine(ref max, Encoding, null);
            return stream; //Get stream from web server;
